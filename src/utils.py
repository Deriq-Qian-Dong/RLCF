...
def merge(total_part, shift, top, eval_cnts, query_dataset_name):
    f_list = []
    for part in range(total_part):
        f0 = open('output/res.top%d.part%d.step%d.%s' % (top, part, eval_cnts, query_dataset_name))
        f_list.append(f0)
    ...
    out = open('output/res.top%d.step%d.%s' % (top, eval_cnts, query_dataset_name), 'w')
    last_q = ''
    ans_list = {}
    while line_list[-1]:
        cur_list = []
