...
class Reranker(nn.Module):
    def __init__(self, args):
        ...
        self.lm = AutoModelForSequenceClassification.from_pretrained(args.reranking_model_name_or_path, num_labels=1, output_hidden_states=True)
...
class QueryGenerator(nn.Module):
    def __init__(self, args):
        ...
        self.lm = T5ForConditionalGeneration.from_pretrained(args.model_name_or_path)
        if args.gradient_checkpoint:
            self.lm.gradient_checkpointing_enable()
        self.args = args
        self.cross_entropy = nn.CrossEntropyLoss(reduction='mean')

    def forward(self, input_ids, attention_mask, max_length=64, do_sample=True, top_k=10, num_return_sequences=5, labels=None):
        # if self.training:
        #     loss = self.lm(input_ids=input_ids, attention_mask=attention_mask, labels=labels).loss
        #     return loss 
        outputs=self.lm.generate(input_ids=input_ids, attention_mask=attention_mask, max_length=max_length, do_sample=do_sample, top_k=top_k, num_return_sequences=num_return_sequences)
        return outputs
