...
def train(  # noqa: C901
    ...
    train_pids: Optional[List[str]] = None,
    recalled_list: Optional[List[List[int]]] = None,
    rewards: Optional[List[float]] = None,
    prompts: Optional[List[str]] = None,
    eval_prompts: Optional[List[str]] = None,
    eval_pids: Optional[List[str]] = None,
    ...
    Args:
        ...
        train_labels (List[Union[str, List[str]]]):
            List of strings used for training target of seq2seq model e.g. T5 etc.
        rewards (List[float]):
    ...
    if reward_fn:
        ...
        if config.train.pipeline=="RecalledPipeline":
            pipeline = get_pipeline(config.train.pipeline)(prompts, recalled_list, config.train.sample_num, max_prompt_length, trainer.tokenizer, promptor.pad_tokens)
        else:
    ...
    # Offline training from the collected samples (e.g. SFT, ILQL)
        ...
        if rewards:
            ...
            trainer.store = get_pipeline(config.train.pipeline)(samples, train_labels, train_pids, max_prompt_length, trainer.tokenizer, promptor.pad_tokens)

    else:
        raise ValueError("Either `samples` or `reward_fn` should be given for training")

    eval_pipeline = get_pipeline(config.train.eval_pipeline)(eval_prompts, eval_labels, eval_pids, max_prompt_length, trainer.tokenizer, promptor.pad_tokens)
