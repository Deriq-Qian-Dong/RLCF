...
class AcceleratePPOTrainer(AccelerateRLTrainer):
    ...
    def __init__(self, config: TRLConfig, **kwargs):
        ...
        rollout_loader: DataLoader = self.store.create_loader(self.config.train.batch_size, shuffle=False)
    ...
    def add_prompt_pipeline(self, pipeline: PromptPipeline):
        ...
        # self.total_steps = min(len(self.prompt_dataloader)*self.config.method.chunk_size//self.config.train.batch_size, self.config.train.total_steps)
        # self.config.train.epochs = self.total_steps // (self.config.method.num_rollouts//self.config.train.batch_size)
    ...
    def make_experience(self, num_rollouts: int = 1024, iter_count: int = 0):  # noqa:
        ...
        with open("output/store_mrr1_%d.tsv"%self.local_rank, "a") as fout:
            # while len(ppo_rl_elements) < num_rollouts:
            for _ in range(num_rollouts//(self.config.method.chunk_size*self.config.train.sample_num)):
                ...
                all_str_samples, all_str_prompts, all_str_outputs = self.decode(
                    prompt_tensors, samples, prompt_sizes
                )
                exp_score_time = time()
                scores = torch.tensor(
                    self.reward_fn(
                ...
                for sample_idx in range(len(scores_lst)//self.sample_num):
                    sample_outputs = all_str_outputs[sample_idx*self.sample_num:sample_idx*self.sample_num+self.sample_num]
                    sample_pids = pids[sample_idx*self.sample_num:sample_idx*self.sample_num+self.sample_num]
                    sample_prompts = all_str_prompts[sample_idx*self.sample_num:sample_idx*self.sample_num+self.sample_num]
                    sample_scores_lst = scores_lst[sample_idx*self.sample_num:sample_idx*self.sample_num+self.sample_num]
                    
                    # for debug
                    # tmp = ""
                    # for qry, pid_, psg, mrr in zip(sample_outputs, sample_pids, sample_prompts, sample_scores_lst): 
                    #     psg = psg[len(instruction)-2:]
                    #     tmp+=str(self.store_for_de_id)+'\t'+qry+'\t'+str(pid_)+'\t'+psg+'\t'+str(mrr)+'\n'
                    # fout.write(tmp)
                    # self.store_for_de_id+=1
                    for qry, pid, psg, mrr in zip(sample_outputs, sample_pids, sample_prompts, sample_scores_lst):
                        if mrr==1:
                            # all_prompts = [prompts.split("Passage:")[-1] for prompts in all_prompts]
                            psg = psg.split("Passage:")[-1]
                            tmp = str(self.store_for_de_id)+'\t'+qry+'\t'+str(pid)+'\t'+psg+'\t'+str(mrr)+'\n'
                            for qry, pid_, psg, mrr in zip(sample_outputs, sample_pids, sample_prompts, sample_scores_lst): 
                                if pid_!=pid:
                                    psg = psg.split("Passage:")[-1]
                                    tmp+=str(self.store_for_de_id)+'\t'+qry+'\t'+str(pid_)+'\t'+psg+'\t'+str(mrr)+'\n'
        ...
        if len(ppo_rl_elements)<num_rollouts:
            ppo_rl_elements = ppo_rl_elements+ppo_rl_elements
